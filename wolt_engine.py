...
import platform # New import to check OS
...
# --- NEW FUNCTION TO FIND A SYSTEM BROWSER ---
def launch_system_browser(p: Playwright):
    """
    Attempts to launch an installed system browser.
    Tries Chrome, then Edge, then Firefox (for Windows/Linux).
    """
    ...
    if system == "Linux":
        # On Linux, we use the explicit path
        try:
            print("Attempting to launch system Chromium...")
            return p.chromium.launch(
                headless=True,
                executable_path="/usr/bin/chromium"
            )
        except Exception:
            print("Linux: Chromium not found. This will likely fail.")
            # Fallback for other Linux distros, just in case
            return p.chromium.launch(headless=True, channel="chrome")

    elif system == "Windows":
        # On Windows, we use channels
        try:
            print("Attempting to launch system Chrome...")
            return p.chromium.launch(headless=True, channel="chrome")
        except Exception:
            print("Chrome not found. Trying Edge...")
        
        try:
            print("Attempting to launch system Edge...")
            return p.chromium.launch(headless=True, channel="msedge")
        except Exception:
            print("Edge not found. Trying Firefox...")
            
        try:
            print("Attempting to launch system Firefox...")
            return p.firefox.launch(headless=True, channel="firefox")
        except Exception:
            print("Firefox not found.")

    # If all checks fail
    raise RuntimeError("Could not find a compatible system browser (Chrome, Edge, or Firefox). Please install one.")


def scrape_wolt(my_address, search_term):
    print(f"--- Starting Wolt Engine (v22 - Cross-Platform) ---")
    ...
    with sync_playwright() as p:
        ...
        try:
            # --- THIS IS THE FIX ---
            # Call our new function to find a browser
            browser = launch_system_browser(p)
